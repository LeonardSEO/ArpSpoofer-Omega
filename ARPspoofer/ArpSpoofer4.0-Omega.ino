...
static void handleWeb() {
  ...
  if (pos) {
    ...
    if (strstr(data, "POST /") != 0) {
      ...
      if (authed) {
        if (strstr(data, "cmd=toggle") != 0)
          paused = !paused;
        if (strstr(data, "cmd=rate") != 0) {
          char *vpos = strstr(data, "val=");
          if (vpos)
            bucket.configure((uint16_t)atoi(vpos + 4));
        }
      }
    }
    ...
    bfill.emit_p(
        PSTR("HTTP/1.0 200 OK\r\nContent-Type:text/html\r\n\r\n"
             ...
             "input,button{background:#222;color:#0f0;border:1px solid "
             "#0f0;padding:10px;margin:5px}</style></head><body>"
             "<h2>OMEGA 4.0</h2>"
             "<p>STS: $S | IP: $D.$D.$D.$D</p><p>TX: $L | $D PPS</p>"
             "<form method='POST'>PW: <input type='password' name='pwd'><br>"
             "<input type='hidden' name='cmd' "
             "value='toggle'><button>$S</button></form>"),
        currentState == STATE_RUN ? (paused ? PSTR("PAUSED") : PSTR("RUNNING"))
                                  : PSTR("BUSY"),
        myIp[0], myIp[1], myIp[2], myIp[3], totalPacketsSent, bucket.pps,
        paused ? PSTR("RESUME") : PSTR("PAUSE"));

    if (!authed)
      bfill.emit_p(PSTR("<p style='color:red'>AUTH ERR</p>"));
...
static void printStatus() {
  ...
  Serial.println(paused ? F("YES") : F("NO"));
  Serial.print(F(" MAC     : "));
  printMac();
  Serial.println();
  Serial.print(F(" IP      : "));
  ...
  Serial.println(bucket.tokens);
  Serial.print(F(" Sent    : "));
  Serial.println(totalPacketsSent);
  Serial.print(F(" DHCP err: "));
  Serial.println(dhcpFailCount);
...
static void stateInit() {
  ...
  printMac();
  Serial.println();
  Serial.print(F("[INIT] STATIC IP: "));
  printIp();
  Serial.println();
  Serial.println(F("[INIT] Hardware OK → Autorun"));

  ledBlink(2, 200, 200);
  
  // BYPASS DHCP, GO DIRECTLY TO ANNOUNCE
...
static void stateDhcp() {
  ...
  if (firstAttempt) {
    ...
    if (dhcpBlocking(DHCP_INIT_ATTEMPTS)) {
      Serial.print(F("[DHCP] Lease OK. IP: "));
      printIp();
      Serial.println();
      dhcpRetries = 0;
      announceSent = 0;
  ...
  if ((uint32_t)(millis() - lastDhcpAttemptMs) >= backoff) {
    ...
    if (dhcpOnce()) {
      ...
      if (USE_STATIC_FALLBACK && dhcpRetries >= DHCP_RETRY_LIMIT) {
        ...
        Serial.print(F("[INIT] Static IP Set: "));
        printIp();
        Serial.println();
        dhcpRetries = 0;
        firstAttempt = true;
...
static void stateAnnounce() {
  ...
  if ((uint32_t)(millis() - nextAnnounceMs) < 0x80000000UL)
    return; // not yet

  sendGarp();
  announceSent++;
  Serial.print(F("[ANN] GARP "));
  Serial.print(announceSent);
  Serial.print(F("/"));
  Serial.println(ANNOUNCE_COUNT);
  ...
  nextAnnounceMs =
      millis() + ANNOUNCE_INTERVAL_MS - ANNOUNCE_JITTER_MS + jitter;
}
